...
export const PloppySettingsSheet = forwardRef<PloppySettingsSheetRef>((_, ref) => {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      setIsLoading(true);
      const connectedAtMount = await isPollinationConnected();
      setIsConnected(connectedAtMount);
      setIsLoading(false); 
    };
  ...
  const handleTogglePloppy = useCallback(async (enabled: boolean) => {
    if (enabled) {
      ...
      if (currentlyConnected) {
        await updateSettings({ ploppyEnabled: true });
      } else {
        // Start auth flow
        Alert.alert(
                ...
                try {
                  ...
                  setTimeout(() => {
                    void (async () => {
                      ...
                      if (nowConnected) {
                        setIsConnected(true);
                        await updateSettings({ ploppyEnabled: true, pollinationConnected: true });
                      }
                    })();
                  }, 2000);
                } catch (error) {
      ...
      await updateSettings({ ploppyEnabled: false });
    }
  }, [t, updateSettings]);
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            await updateSettings({ ploppyEnabled: false, pollinationConnected: false });
          }
        },
      ]
    );
