...
let healthConnectModule: typeof import("react-native-health-connect") | null =
  null;
...
export async function requestHealthConnectPermissions(): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      errorLogger.warn(
        "Health Connect init skipped before permission request:",
        error,
      );
    }
...
export async function getRecentWeights(
  ...
  try {
    ...
    healthConnectLogger.debug(
      `Received ${result.records.length} weight records`,
    );
...
export async function getRecentBodyFat(
  daysBack: number = 30,
): Promise<HealthConnectBodyFat[]> {
  if (Platform.OS !== "android") return [];
  ...
  try {
    ...
    healthConnectLogger.debug(
      `Received ${result.records.length} body fat records`,
    );

    return result.records.map((record: HealthConnectBodyFatRecord) => ({
      id:
        record.metadata?.id || `hc-bodyfat-${new Date(record.time).getTime()}`,
      time: new Date(record.time),
...
export async function getRecentWorkouts(
  ...
  try {
    ...
    result.records.forEach((r) => {
      healthConnectLogger.debug(
        `- ${EXERCISE_TYPE_NAMES[r.exerciseType as number] || r.exerciseType} | Date: ${new Date(r.startTime).toLocaleDateString()} ${new Date(r.startTime).toLocaleTimeString()}`,
      );
    });
    ...
    for (const record of result.records.slice(0, MAX_HEALTH_CONNECT_WORKOUTS)) {
      ...
      const displayTitle =
        record.title && record.title !== "null" ? record.title : typeName;

      // Fetch distance for this workout session
      let distance: number | undefined;
      try {
        const distanceMeters = await getDistanceForWorkoutInternal(
          hc,
          start,
          end,
        );
 ...
 */
export async function getDistanceForWorkout(
  startTime: Date,
  endTime: Date,
): Promise<number> {
  ...
  try {
    ...
    const distance = await getDistanceForWorkoutInternal(
      hc,
      startTime,
      endTime,
    );
